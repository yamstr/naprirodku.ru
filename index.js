...
app.get('/articles/:id/', function (req, res, next) {
    db.task(function (t) {
            return t.one('SELECT * FROM articles WHERE id = ${id}', {id: parseInt(req.params.id)})
                .then(function (article) {
                    article.body = marked(article.body);
                    return t.one('SELECT *, position[0] AS lat, position[1] AS lng FROM places WHERE id = ${id}', {id: article.place_id})
                        .then(function (place) {
                            place.position = {
                                lat: place.lat,
                                lng: place.lng
        ...
        .catch(function (error) {
            // perhaps should report the error here also ;)
            next();
        });
});
